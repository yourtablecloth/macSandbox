...
// Custom context with rdpContext as the first member (libfreerdp convention).
typedef struct {
    rdpContext _ctx;
    const char *outpath;
    int paints;
...
static BOOL cf_authenticate(freerdp *instance, char **user, char **pass, char **domain) {
    ...
    return TRUE; // use the credentials from settings
}
...
int cfreerdp_capture(const char *host, int port, const char *outpath) {
    ...
    // Force the legacy GDI path (gfx disabled) — the framebuffer is updated via EndPaint.
    ...
    HANDLE handles[64];
    int loops = 0;
    int settle = -1;          // number of extra processing loops after the first paint
    int captured = 0;
    while (!freerdp_shall_disconnect_context(instance->context)) {
        ...
        if (cf->paints > 0) {
            ...
            // Process ~50 more seconds after the first paint to capture once the first-login profile loading is done.
            if (settle >= 250) {
        ...
        if (++loops > 300) break; // safety guard (~60 seconds max)
    }
